...
class TerminalCoreUnitTests::ConptyRoundtripTests final
{
    ...
    TEST_METHOD_SETUP(MethodSetup)
    {
        ...
        VERIFY_ARE_EQUAL({}, currentBuffer.GetTextBuffer().GetCursor().GetPosition());
...
void ConptyRoundtripTests::BreakLinesOnCursorMovement()
{
    ...
    verifyBuffer(*term->_altBuffer, term->_mutableViewport.ToInclusive());
}
...
void ConptyRoundtripTests::TestCursorInDeferredEOLPositionOnNewLineWithSpaces()
{
    ...
    auto verifyBuffer = [&](const TextBuffer& tb, til::CoordType bottomRow) {
        ...
        VERIFY_ARE_EQUAL(({ 1, bottomRow }), cursorPos);
...
void ConptyRoundtripTests::SimpleAltBufferTest()
{
    ...
    verifyBuffer(*hostTb, si.GetViewport().ToInclusive(), Frame::InMainBufferBefore);

    Log::Comment(L"Painting the frame");
    VERIFY_SUCCEEDED(renderer.PaintFrame());
    Log::Comment(L"========== Checking the terminal buffer state (InMainBufferBefore) ==========");
    VERIFY_ARE_EQUAL(0, term->_GetMutableViewport().Top());
    verifyBuffer(*termTb, term->_GetMutableViewport().ToInclusive(), Frame::InMainBufferBefore);
    ...
    verifyBuffer(*hostAltTb, siAlt.GetViewport().ToInclusive(), Frame::InAltBufferBefore);

    Log::Comment(L"Painting the frame");
    VERIFY_SUCCEEDED(renderer.PaintFrame());
    Log::Comment(L"========== Checking the terminal buffer state (InAltBufferBefore) ==========");
    VERIFY_ARE_EQUAL(0, term->_GetMutableViewport().Top());
    verifyBuffer(*termAltTb, term->_GetMutableViewport().ToInclusive(), Frame::InAltBufferBefore);
    ...
    verifyBuffer(*hostAltTb, siAlt.GetViewport().ToInclusive(), Frame::InAltBufferAfter);
    Log::Comment(L"Painting the frame");
    VERIFY_SUCCEEDED(renderer.PaintFrame());

    Log::Comment(L"========== Checking the terminal buffer state (InAltBufferAfter) ==========");
    VERIFY_ARE_EQUAL(0, term->_GetMutableViewport().Top());
    verifyBuffer(*termAltTb, term->_GetMutableViewport().ToInclusive(), Frame::InAltBufferAfter);
    ...
    verifyBuffer(*hostTb, si.GetViewport().ToInclusive(), Frame::InMainBufferAfter);
    Log::Comment(L"Painting the frame");
    VERIFY_SUCCEEDED(renderer.PaintFrame());

    Log::Comment(L"========== Checking the terminal buffer state (InMainBufferAfter) ==========");
    VERIFY_ARE_EQUAL(0, term->_GetMutableViewport().Top());
    verifyBuffer(*termTb, term->_GetMutableViewport().ToInclusive(), Frame::InMainBufferAfter);
}
...
void ConptyRoundtripTests::AltBufferToAltBufferTest()
{
    ...
    verifyBuffer(*hostAltTb, siAlt->GetViewport().ToInclusive(), Frame::StillInAltBuffer);

    Log::Comment(L"========== Checking the terminal buffer state (StillInAltBuffer) ==========");
    verifyBuffer(*termAltTb, term->_GetMutableViewport().ToInclusive(), Frame::StillInAltBuffer);
}
